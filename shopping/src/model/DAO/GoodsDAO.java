...
public class GoodsDAO extends DataBaseInfo{
	...
	public List<ProdReviewDTO> prodReviewSelect(String prodNum) {
		...
		sql = "select rpad(substr(m.mem_id,1,3),length(m.mem_id),'*') mem_id, review_content, review_img, review_date from member m, purchase p, review r where m.mem_id = p.mem_id and p.purchase_num = r.purchase_num and r.prod_num = ?";
		getConnect();
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, prodNum);
